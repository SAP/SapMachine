...
public class TestPosix {
    ...
    private FileSystem createTestZipFile(Path zpath, Map<String, Object> env) throws IOException {
        ...
        FileSystem fs = zipFSP.newFileSystem(zpath, opts);
    ...
    private FileSystem createEmptyZipFile(Path zpath, Map<String, Object> env) throws IOException {
        ...
        opts.putAll(env);
        opts.put("create", true);
        return zipFSP.newFileSystem(zpath, opts);
    }
    ...
    public void testDefault() throws IOException {
        ...
        try (FileSystem zip = zipFSP.newFileSystem(ZIP_FILE, ENV_DEFAULT)) {
            checkEntries(zip, checkExpects.permsInZip);
        }
        // check entries on zipfs with posix options
        try (FileSystem zip = zipFSP.newFileSystem(ZIP_FILE, ENV_POSIX)) {
    ...
    public void testCopy() throws IOException {
        ...
        try (FileSystem zip = zipFSP.newFileSystem(ZIP_FILE_COPY, ENV_DEFAULT)) {
            checkEntries(zip, checkExpects.permsInZip);
        }
        // check entries on copied zipfs with posix options
        try (FileSystem zip = zipFSP.newFileSystem(ZIP_FILE_COPY, ENV_POSIX)) {
            checkEntries(zip, checkExpects.permsPosix);
        }
    }
    ...
    public void testPosixDefaults() throws IOException {
        ...
        try (FileSystem zipIn = zipFSP.newFileSystem(ZIP_FILE, ENV_POSIX)) {
        ...
        try (FileSystem zipIn = zipFSP.newFileSystem(ZIP_FILE, Map.of("enablePosixFileAttributes", true,
