...
StatEntry*  MallocStatisticImpl::record_allocation_size(size_t to_add, int nr_of_frames, address* frames) {
  ...
  Locker locker(&_stack_maps_lock[idx]._lock);
...
void MallocStatisticImpl::record_allocation(void* ptr, uint64_t hash, int nr_of_frames, address* frames) {
  ...
  Locker locker(&_alloc_maps_lock[idx]._lock);
  ...
  while (*entry != NULL) {
    if ((*entry)->hash() == hash) {
      ...
      *entry = next;
      ...
      Locker locker2(&_stack_maps_lock[idx2]._lock);
...
static void print_allocation_stats(outputStream* st, Allocator** allocs, int* masks, int* sizes,
...
bool MallocStatisticImpl::dump(outputStream* msg_stream, outputStream* dump_stream, DumpSpec const& spec) {
  ...
  Locker lock(spec._on_error ? NULL : &_malloc_stat_lock._lock);
  ...
  // field.
  for (int i = 0; i < NR_OF_STACK_MAPS; ++i) {
    Locker locker(&_stack_maps_lock[i]._lock);
  }
