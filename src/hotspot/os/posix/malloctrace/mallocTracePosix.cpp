...
ALWAYSINLINE int MallocStatisticImpl::capture_stack(address* frames, address real_func, address caller) {
  ...
  if (_detailed_stats) {
    Atomic::add(Ticks::now().nanoseconds() - ticks, &_stack_walk_time);
    Atomic::add((uint64_t) 1, &_stack_walk_count);
  }
...
bool MallocStatisticImpl::should_track(uint64_t hash) {
  if (_detailed_stats) {
    if ((hash & _to_track_mask) < _to_track_limit) {
      Atomic::add((uint64_t) 1, &_tracked_ptrs);
    } else {
      Atomic::add((uint64_t) 1, &_not_tracked_ptrs);
    }
  }
  ...
  while (entry != NULL) {
    if (entry->hash() == hash) {
      ...
      for (int i = 0; i < nr_of_frames; ++i) {
        ...
        print_function_and_library_name(&ss, frames[i], tmp, sizeof(tmp), true, true, false);
        ss.cr();
      }
      ...
      for (int i = 0; i < stat_entry->nr_of_frames(); ++i) {
        ...
        if (print_function_and_library_name(&ss, frame, tmp, sizeof(tmp), true, true, false)) {
          ss.cr();
        } else {
...
StatEntry* MallocStatisticImpl::record_free(void* ptr, uint64_t hash, size_t size) {
  ...
  assert(map._entries != NULL, "Must be initialized");
  ...
  if (_detailed_stats) {
    Atomic::add((uint64_t) 1, &_failed_frees);
  }
...
static void print_frame(outputStream* st, address frame) {
  ...
  if (print_function_and_library_name(st, frame, tmp, sizeof(tmp), true, true, false)) {
    st->cr();
  } else {
    CodeBlob* blob = CodeCache::find_blob((void*) frame);
...
bool MallocStatisticImpl::dump_entry(outputStream* st, StatEntryCopy* entry, int index,
  ...
  if (is_non_empty_string(filter)) {
    ...
    for (int i = 0; i < entry->_entry->nr_of_frames(); ++i) {
      ...
      if ((frame == NULL) || filter_cache->contains(frame)) {
        continue;
      }
...
bool MallocStatisticImpl::dump(outputStream* msg_stream, outputStream* dump_stream, DumpSpec const& spec) {
  ...
  if (spec._on_error) {
    if (_initialized) {
      ...
      if (Atomic::cmpxchg(true, &_rainy_day_fund_used, false) == true) {
...
class MallocTraceDumpTask : public PeriodicTask {
private:
  uint64_t _base_period;
  uint64_t _period_ticks;
  uint64_t _delay_ticks_left;
  uint64_t _period_ticks_left;
  uint64_t _dumps_left;

  uint64_t get_base_period(uint64_t delay, uint64_t period);

public:
  MallocTraceDumpTask(uint64_t delay, uint64_t period) : 
    PeriodicTask(1000 * get_base_period(delay, period)),
    _base_period(get_base_period(delay, period)),
    _period_ticks(period / _base_period),
    _delay_ticks_left(delay / _base_period),
    _period_ticks_left(_period_ticks),
    _dumps_left(MallocTraceDumpCount) {
  }

  virtual void task();
};

uint64_t MallocTraceDumpTask::get_base_period(uint64_t delay, uint64_t period) {
  if (delay == 0) {
    return period;
  }

  // Return the gcd using euclidian algorithm.
  uint64_t t1 = MAX2(delay, period);
  uint64_t t2 = MIN2(delay, period);
...
void MallocTraceDumpTask::task() {
  if (_dumps_left <= 0) {
    disenroll();
  } else if (_delay_ticks_left > 0) {
    --_delay_ticks_left;

    if (_delay_ticks_left == 0) {
...
static void enable_delayed_dump() {
  if (MallocTraceDumpCount > 0) {
    ...
    uint64_t period = MAX2((uint64_t) 1, parse_timespan(MallocTraceDumpInterval));

    mallocStatImpl::MallocTraceDumpTask* task = new mallocStatImpl::MallocTraceDumpTask(delay, period);
    task->enroll();
  }
}
