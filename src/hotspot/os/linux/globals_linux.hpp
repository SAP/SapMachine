...
#define RUNTIME_OS_FLAGS(develop,                                       \
  ...
  product(bool, HiMemReport, false,                                     \
         "A high memory report will be generated when process rss+swap reaches either one of 33%, 50%, 75%, 85%, 90%, 95% of a maximum.\n" \
         "If the VM is containerized, that maximum is the container memory limit at VM start. If the VM is not containerized, that maximum is the total size of physical memory. The maximum can be overridden with -XX:HiMemReportMaximum=<size>.\n" \
         "Per default, the report is printed to stderr, but that can be redirected with -XX:HiMemReportFile=<file>.") \
	product(size_t, HiMemReportMax, 0,                                    \
          "Overrides the maximum to be used with HiMemReport.")         \
  product(ccstr, HiMemReportDir, NULL,                                  \
			    "Redirect the reports generated by HiMemReport to this directory (<dir>/sapmachine_himem_alert_<pid>_<percentage>.log.") \
  product(ccstr, HiMemReportExec, NULL,                                 \
			    "Call an outside program on himem report. It will be given, as arguments, the pid and the alert percentage.") \
